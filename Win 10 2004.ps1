...
switch ([Environment]::Is64BitOperatingSystem)
{
	$false
	{
		if ($RU)
		{
			Write-Warning -Message "Dieser Script unterstützt leider nur die x64 Version von Windows 10"
		}
		else
		{
...
switch ([IntPtr]::Size -eq 8)
{
	$false
	{
		if ($RU)
		{
			Write-Warning -Message "Dieser Script benötigt die x64 Version von PowerShell"
		}
		else
		{
...
if ($psISE)
{
	if ($RU)
	{
		Write-Warning -Message "Dieser Script läuft nicht PowerShell ISE"
	}
	else
	{
...
switch ($Result)
{
	...
	"1"
	{
		if ($RU)
		{
			...
			$Message = "Um alle Wiederherstellungpunkte zu entfernen drücken sie die benötigte Taste"
			$Options = "&Löschen", "&Überspringen"
		}
		else
		{
			$Title = "Restore point"
		...
		switch ($Result)
		{
			...
			"1"
			{
				if ($RU)
				{
					Write-Verbose -Message "Übersprungen" -Verbose
				}
				else
				{
...
Write-Host "Downloading Edge" -NoNewline
(New-Object System.Net.WebClient).DownloadFile("http://go.microsoft.com/fwlink/?LinkID=2093437", "$env:LOCALAPPDATA\Temp\MicrosoftEdgeEnterpriseX64.msi")
Write-host "`r - Success!"
Write-Host "Downloading GeForce NOW" -NoNewline
(New-Object System.Net.WebClient).DownloadFile("https://download.nvidia.com/gfnpc/GeForceNOW-release.exe", "$env:LOCALAPPDATA\Temp\GeForceNOW-release.exe")
Write-host "`r - Success!"
Write-Host "Downloading Parsec" -NoNewline
(New-Object System.Net.WebClient).DownloadFile("https://builds.parsecgaming.com/package/parsec-windows.exe", "$env:LOCALAPPDATA\Temp\parsec-windows.exe")
Write-host "`r - Success!"
...
start-process -filepath "C:\Windows\System32\msiexec.exe" -ArgumentList '/qn /i "$env:LOCALAPPDATA\Temp\MicrosoftEdgeEnterpriseX64.msi"' -Wait
Start-Process -FilePath "$env:LOCALAPPDATA\Temp\directx_Jun2010_redist.exe" -ArgumentList '/T:$env:LOCALAPPDATA\Temp\DirectX /Q'-wait
